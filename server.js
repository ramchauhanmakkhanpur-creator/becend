...
// NOTE: Render/Koyeb par ye local file restart hone par ud jayegi.
// Production ke liye MongoDB ka use zyada safe rahega.
...
const homeKeywords = [
    "vlog india", "podcast hindi", "tech reviews india", 
    "indian web series", "roast video hindi", "stand up comedy hindi", 
    "travel vlog india", "shark tank india"
];

// Video Fetch API - Fixed
app.get('/api/videos', async (req, res) => {
    const { q = 'trending', page = 1, type = 'search' } = req.query;
    
    try {
        let searchQuery = q;
        const pageSuffix = parseInt(page) > 1 ? ` part ${page}` : '';

        if (type === 'home') {
            const randomKeyword = homeKeywords[Math.floor(Math.random() * homeKeywords.length)];
            searchQuery = `${randomKeyword}${pageSuffix}`;
        } else if (type === 'shorts') {
            searchQuery = `#shorts trending${pageSuffix}`;
        } else {
            searchQuery = `${q}${pageSuffix}`;
        }

        const r = await ytSearch(searchQuery);
        // FIXED: Safe check for r.videos to prevent undefined crash
        let videos = r?.videos || [];

        if (type === 'home') {
            videos = videos.filter(v => v.seconds > 240); // 4 minute se badi
        } else if (type === 'shorts') {
            videos = videos.filter(v => v.seconds <= 60); // 60 second se kam
        }
        ...
        // Error array bhejen taaki frontend handle kar sake
        res.status(500).json({ results: [], error: 'Server Error Fetching Videos' });
    }
});
